...
while true; do
    ...
    if [[ $GPUTEMP -gt 40 ]]; then
        /runcurrent-system/sw/bin/nvidia-settings -a '[fan:0]/GPUTargetFanSpeed=45' && /run/current-system/swbin/nvidia-settings -a '[fan:1]/GPUTargetFanSpeed=45'
    fi
